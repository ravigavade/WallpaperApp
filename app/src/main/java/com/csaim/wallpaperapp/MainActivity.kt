...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val wallpaperManager = LatestWallpaperManager()
...
//        }
        ...
        lifecycleScope.launch {
            withContext(Dispatchers.IO) {
                wallpapers = wallpaperManager.retrieveLatestsWallpaper(apiKey)
                Log.d("fucal","calling")
            }

            binding.recyclerView.layoutManager = LinearLayoutManager(this@MainActivity,LinearLayoutManager.HORIZONTAL,false)
            binding.recyclerView.adapter = LatersWallpaperAdapter(this@MainActivity, wallpapers) { imageUrl ->
